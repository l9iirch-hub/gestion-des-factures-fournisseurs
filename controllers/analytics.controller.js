...
const getSupplierStats = async (req, res) => {
  ...
  const invoices = await Invoice.find({ supplierId: supplier._id, userId: req.user.id });
  const payments = await Payment.find({ invoiceId: { $in: invoices.map(i => i._id) } });
  ...
  const invoicesByStatus = { unpaid: 0, partially_paid: 0, paid: 0, overdue: 0 };
  
  for (const invoice of invoices) {
    const invoicePayments = payments.filter(p => p.invoiceId.toString() === invoice._id.toString());
    ...
    if (dueDate < today && status !== 'paid') { status = 'overdue'; overdueCount++; }
...
const getDashboard = async (req, res) => {
  ...
  for (const invoice of invoices) {
    const invoicePayments = payments.filter(p => p.invoiceId.toString() === invoice._id.toString());
    const paidAmount = invoicePayments.reduce((sum, p) => sum + p.amount, 0);
    if (!supplierSpending[invoice.supplierId]) supplierSpending[invoice.supplierId] = 0;
    supplierSpending[invoice.supplierId] += paidAmount;
  }
  
  const topSuppliers = await Promise.all(
    Object.entries(supplierSpending).sort((a, b) => b[1] - a[1]).slice(0, 3).map(async ([supplierId, amount]) => {
      const supplier = await Supplier.findById(supplierId);
      return { supplierId, supplierName: supplier ? supplier.name : 'Inconnu', totalSpent: amount };
    })
  );
